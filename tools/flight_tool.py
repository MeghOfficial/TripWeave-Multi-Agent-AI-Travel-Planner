import os 
import re 
import certifi
import airportsdata
import pycountry
import requests
from dotenv import load_dotenv

load_dotenv()

os.environ["SSL_CERT_FILE"] = certifi.where()
os.environ["REQUESTS_CA_BUNDLE"] = certifi.where()

API_KEY = os.getenv("AVIATIONSTACK_API_KEY")

# Default origin when user says only destination, e.g. "Japan trip"
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "DEL")

BASE_URL = "https://api.aviationstack.com/v1/flights"

AIRPORTS = airportsdata.load("IATA")


COUNTRY_ALIASES = {
    "usa": "US",
    "u.s.a": "US",
    "u.s.": "US",
    "america": "US",
    "united states": "US",
    "uk": "GB",
    "u.k.": "GB",
    "britain": "GB",
    "england": "GB",
    "uae": "AE",
    "dubai": "AE",
    "south korea": "KR",
    "korea": "KR",
    "russia": "RU",
    "vietnam": "VN",
    "bangladesh": "BD",
    "india": "IN",
    "japan": "JP",
    "china": "CN",
    "singapore": "SG",
    "malaysia": "MY",
    "thailand": "TH",
    "indonesia": "ID",
    "nepal": "NP",
    "qatar": "QA",
    "saudi arabia": "SA",
    "turkey": "TR",
    "canada": "CA",
    "australia": "AU",
    "germany": "DE",
    "france": "FR",
    "italy": "IT",
    "spain": "ES",
}


# Preferred main airport for country-level search
COUNTRY_MAIN_AIRPORT = {
    "BD": "DAC",
    "IN": "DEL",
    "JP": "NRT",
    "US": "JFK",
    "GB": "LHR",
    "AE": "DXB",
    "SG": "SIN",
    "MY": "KUL",
    "TH": "BKK",
    "ID": "CGK",
    "CN": "PEK",
    "KR": "ICN",
    "NP": "KTM",
    "QA": "DOH",
    "SA": "JED",
    "TR": "IST",
    "CA": "YYZ",
    "AU": "SYD",
    "DE": "FRA",
    "FR": "CDG",
    "IT": "FCO",
    "ES": "MAD",
}



CITY_MAIN_AIRPORT = {
    "dhaka": "DAC",
    "delhi": "DEL",
    "new delhi": "DEL",
    "mumbai": "BOM",
    "kolkata": "CCU",
    "chennai": "MAA",
    "bangalore": "BLR",
    "bengaluru": "BLR",
    "tokyo": "NRT",
    "osaka": "KIX",
    "kyoto": "KIX",
    "new york": "JFK",
    "london": "LHR",
    "dubai": "DXB",
    "singapore": "SIN",
    "kuala lumpur": "KUL",
    "bangkok": "BKK",
    "doha": "DOH",
    "istanbul": "IST",
    "toronto": "YYZ",
    "sydney": "SYD",
    "paris": "CDG",
    "rome": "FCO",
    "madrid": "MAD",
    "frankfurt": "FRA",
}

def clean_text(text: str) -> str:
    # Convert text to lowercase and remove extra spaces
    text = text.lower().strip()

    # Remove special characters
    text = re.sub(r"[^a-z0-9\s]", " ", text)

    # Replace multiple spaces with one space
    text = re.sub(r"\s+", " ", text)

    # Words that are not useful for finding locations
    stop_words = [
        "flight", "flights", "ticket", "tickets", "trip", "travel",
        "plan", "complete", "days", "day", "including", "hotel",
        "hotels", "sightseeing", "under", "budget", "info", "information"
    ]

    # Remove common words from the text
    words = [w for w in text.split() if w not in stop_words]

    return " ".join(words).strip()


def country_name_to_code(text: str):
    # Clean the input text first
    text = clean_text(text)

    # Check our custom country aliases
    if text in COUNTRY_ALIASES:
        return COUNTRY_ALIASES[text]

    try:
        # Try to find the country using pycountry
        country = pycountry.countries.lookup(text)
        return country.alpha_2
    except LookupError:
        pass

    # Check if a country name is inside a longer sentence
    for country in pycountry.countries:
        country_name = country.name.lower()

        if country_name in text:
            return country.alpha_2

    # Check country aliases inside longer text
    for alias, code in COUNTRY_ALIASES.items():
        if alias in text:
            return code

    return None


def airport_country_matches(airport: dict, country_code: str) -> bool:
    # Get the airport country from the airport data
    airport_country = str(
        airport.get("country", "")
    ).upper().strip()

    # Direct country code match
    if airport_country == country_code:
        return True

    try:
        # Also check the full country name
        country = pycountry.countries.get(alpha_2=country_code)

        if country and airport_country.lower() == country.name.lower():
            return True

    except Exception:
        pass

    return False


def get_best_airport_for_country(country_code: str):
    # First check if we already have a preferred airport
    preferred = COUNTRY_MAIN_AIRPORT.get(country_code)

    if preferred and preferred in AIRPORTS:
        return preferred

    candidates = []

    # Search airports belonging to this country
    for iata, airport in AIRPORTS.items():

        if not iata:
            continue

        if airport_country_matches(airport, country_code):
            name = str(airport.get("name", "")).lower()
            city = str(airport.get("city", "")).lower()

            # Give points to better airport choices
            score = 0

            if "international" in name:
                score += 50

            if "intl" in name:
                score += 40

            if "capital" in name:
                score += 20

            if city:
                score += 5

            candidates.append((score, iata))

    # No airport found
    if not candidates:
        return None

    # Sort by score and return the best airport
    candidates.sort(reverse=True)

    return candidates[0][1]


def resolve_location_to_iata(location: str):
    """
    Converts country/city/airport/IATA into IATA code.

    Examples:
    Bangladesh -> DAC
    Japan -> NRT
    Dhaka -> DAC
    Tokyo -> NRT
    DAC -> DAC
    """

    # Return nothing if location is empty
    if not location:
        return None

    raw_location = location.strip()

    # Check if the input is already an IATA code
    if re.fullmatch(r"[A-Za-z]{3}", raw_location):
        code = raw_location.upper()

        if code in AIRPORTS:
            return code

    # Clean the location text
    location_clean = clean_text(raw_location)

    if not location_clean:
        return None

    # Check preferred airport for the city
    if location_clean in CITY_MAIN_AIRPORT:
        return CITY_MAIN_AIRPORT[location_clean]

    # Check if the location is a country
    country_code = country_name_to_code(location_clean)

    if country_code:
        airport = get_best_airport_for_country(country_code)

        if airport:
            return airport

    # Search for the city in the airport database
    city_matches = []

    for iata, airport in AIRPORTS.items():
        city = str(
            airport.get("city", "")
        ).lower().strip()

        name = str(
            airport.get("name", "")
        ).lower().strip()

        # Calculate a score for each matching airport
        score = 0

        # Exact city match gets the highest score
        if city == location_clean:
            score += 100

        # Partial city match
        elif location_clean in city:
            score += 70

        # Match location with airport name
        if location_clean in name:
            score += 50

        # Prefer international airports
        if "international" in name:
            score += 10

        if score > 0:
            city_matches.append((score, iata))

    # Return the airport with the highest score
    if city_matches:
        city_matches.sort(reverse=True)
        return city_matches[0][1]

    return None


def find_location_mentions(query: str):
    """
    Finds country or city names inside a natural language query.
    """

    q = query.lower()
    mentions = []

    # Check our custom country aliases
    for alias in COUNTRY_ALIASES:
        if re.search(rf"\b{re.escape(alias)}\b", q):
            mentions.append(alias)

    # Check country names from pycountry
    for country in pycountry.countries:
        name = country.name.lower()

        if len(name) >= 4 and re.search(
            rf"\b{re.escape(name)}\b",
            q
        ):
            mentions.append(name)

    # Check cities from our preferred airport map
    for city in CITY_MAIN_AIRPORT:
        if re.search(rf"\b{re.escape(city)}\b", q):
            mentions.append(city)

    # Remove duplicate locations while keeping their order
    unique_mentions = []

    for item in mentions:
        if item not in unique_mentions:
            unique_mentions.append(item)

    return unique_mentions


def parse_route(query: str):
    """
    Returns:
    dep_iata, arr_iata

    Can return:
    None, None  -> global live flights
    DAC, NRT    -> filtered route
    DAC, None   -> all flights from DAC
    None, NRT   -> all flights to NRT
    """

    q = query.strip()
    q_lower = q.lower()

    # Check for requests asking for worldwide/global flights
    global_keywords = [
        "all country",
        "all countries",
        "global flight",
        "global flights",
        "all flight",
        "all flights",
        "worldwide flight",
        "worldwide flights",
    ]

    if any(keyword in q_lower for keyword in global_keywords):
        return None, None

    # Find possible 3-letter IATA codes
    # Keep only codes that exist in our airport database
    codes = [
        code
        for code in re.findall(r"\b[A-Z]{3}\b", q)
        if code in AIRPORTS
    ]

    # If two airport codes are found, use them as origin and destination
    if len(codes) >= 2:
        dep = codes[0].upper()
        arr = codes[1].upper()

        return dep, arr

    # Check pattern: from X to Y
    match = re.search(
        r"\bfrom\s+(.+?)\s+\bto\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)",
        q_lower,
    )

    if match:
        origin_text = match.group(1)
        dest_text = match.group(2)

        # Convert origin and destination into IATA codes
        dep_iata = resolve_location_to_iata(origin_text)
        arr_iata = resolve_location_to_iata(dest_text)

        return dep_iata, arr_iata

    # Check pattern: to Y from X
    match = re.search(
        r"\bto\s+(.+?)\s+\bfrom\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)",
        q_lower,
    )

    if match:
        dest_text = match.group(1)
        origin_text = match.group(2)

        dep_iata = resolve_location_to_iata(origin_text)
        arr_iata = resolve_location_to_iata(dest_text)

        return dep_iata, arr_iata

    # Check pattern: flights from X
    match = re.search(
        r"\bfrom\s+(.+?)(?:[.!?]|$)",
        q_lower
    )

    if match:
        origin_text = match.group(1)
        dep_iata = resolve_location_to_iata(origin_text)

        return dep_iata, None

    # Check pattern: flights to X
    match = re.search(
        r"\bto\s+(.+?)(?:[.!?]|$)",
        q_lower
    )

    if match:
        dest_text = match.group(1)
        arr_iata = resolve_location_to_iata(dest_text)

        return None, arr_iata

    # If no route pattern matched, find locations in the query
    mentions = find_location_mentions(q)

    # If two locations are found, use them as origin and destination
    if len(mentions) >= 2:
        dep_iata = resolve_location_to_iata(mentions[0])
        arr_iata = resolve_location_to_iata(mentions[1])

        return dep_iata, arr_iata

    # If only one location is found, use the default origin
    if len(mentions) == 1:
        arr_iata = resolve_location_to_iata(mentions[0])

        return DEFAULT_ORIGIN_IATA, arr_iata

    return None, None


def format_flight(flight: dict):
    # Get basic flight information
    airline = flight.get("airline", {}).get("name") or "Unknown airline"
    flight_number = (
        flight.get("flight", {}).get("iata")
        or "Unknown flight number"
    )
    status = flight.get("flight_status") or "Unknown"

    # Get departure and arrival information
    dep = flight.get("departure", {}) or {}
    arr = flight.get("arrival", {}) or {}

    # Departure details
    dep_airport = dep.get("airport") or "Unknown departure airport"
    dep_iata = dep.get("iata") or "Unknown"
    dep_terminal = dep.get("terminal") or "N/A"
    dep_gate = dep.get("gate") or "N/A"
    dep_scheduled = dep.get("scheduled") or "Unknown"

    dep_delay = dep.get("delay")
    dep_delay_text = (
        f"{dep_delay} minutes"
        if dep_delay is not None
        else "N/A"
    )

    # Arrival details
    arr_airport = arr.get("airport") or "Unknown arrival airport"
    arr_iata = arr.get("iata") or "Unknown"
    arr_terminal = arr.get("terminal") or "N/A"
    arr_gate = arr.get("gate") or "N/A"
    arr_scheduled = arr.get("scheduled") or "Unknown"

    arr_delay = arr.get("delay")
    arr_delay_text = (
        f"{arr_delay} minutes"
        if arr_delay is not None
        else "N/A"
    )

    # Return the flight in a simple readable format
    return f"""
    Airline: {airline}
    Flight: {flight_number}
    Status: {status}

    Departure:
    - Airport: {dep_airport}
    - IATA: {dep_iata}
    - Terminal: {dep_terminal}
    - Gate: {dep_gate}
    - Scheduled: {dep_scheduled}
    - Delay: {dep_delay_text}

    Arrival:
    - Airport: {arr_airport}
    - IATA: {arr_iata}
    - Terminal: {arr_terminal}
    - Gate: {arr_gate}
    - Scheduled: {arr_scheduled}
    - Delay: {arr_delay_text}
    """.strip()


def search_flights(query: str, limit: int = 10):
    # Check if the AviationStack API key is available
    if not API_KEY:
        return (
            "Flight API error: AVIATIONSTACK_API_KEY is missing.\n"
            "Please add this in your .env file:\n"
            "AVIATIONSTACK_API_KEY=your_api_key_here"
        )

    # Find origin and destination from the user's query
    dep_iata, arr_iata = parse_route(query)

    # Prepare parameters for the API request
    params = {
        "access_key": API_KEY,
        "limit": min(limit, 15),
    }

    # Add origin only if it was found
    if dep_iata:
        params["dep_iata"] = dep_iata

    # Add destination only if it was found
    if arr_iata:
        params["arr_iata"] = arr_iata

    try:
        # Send request to AviationStack
        response = requests.get(
            BASE_URL,
            params=params,
            timeout=30
        )

        # Convert API response into Python data
        data = response.json()

    except requests.exceptions.RequestException as e:
        return f"Flight API request failed: {e}"

    except ValueError:
        return "Flight API returned invalid JSON."

    # Check if AviationStack returned an error
    if "error" in data:
        error = data["error"]

        return (
            "Flight API error:\n"
            f"Code: {error.get('code', 'Unknown')}\n"
            f"Message: {error.get('message', 'Unknown error')}"
        )

    # Get flight data from the response
    flight_data = data.get("data", [])

    # Handle case when no flights are found
    if not flight_data:
        route_text = ""

        if dep_iata and arr_iata:
            route_text = f" for route {dep_iata} to {arr_iata}"

        elif dep_iata:
            route_text = f" from {dep_iata}"

        elif arr_iata:
            route_text = f" to {arr_iata}"

        return (
            f"No live flight data found{route_text}.\n\n"
            "Note: AviationStack provides live/status flight data, not ticket prices. "
            "For actual fare prices, use a flight-pricing API such as Amadeus."
        )

    # Create a short description of the searched route
    route_info = "Global live flights"

    if dep_iata and arr_iata:
        route_info = f"Live flights from {dep_iata} to {arr_iata}"

    elif dep_iata:
        route_info = f"Live flights from {dep_iata}"

    elif arr_iata:
        route_info = f"Live flights to {arr_iata}"

    # Format only the requested number of flights
    formatted_flights = [
        format_flight(flight)
        for flight in flight_data[:limit]
    ]

    # Return all formatted flights
    return f"{route_info}\n\n" + "\n\n---\n\n".join(formatted_flights)

